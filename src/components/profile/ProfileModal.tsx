...
export default function ProfileModal({ isOpen, onClose, userEmail, currentInitials, onSaveProfile }: ProfileModalProps) {
  ...
  // États du formulaire allégés (état 'role' retiré car inutilisé)
  ...
  // Extraction du rendu pour éliminer le ternaire imbriqué
  const renderContent = () => {
    if (isLoading) {
      return (
        ...
        <form id="profile-form" onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pb-6 border-b border-slate-100 dark:border-slate-800">
            <div className="md:col-span-3 mt-2">
              <div className="block text-sm font-bold text-slate-800 dark:text-slate-200 mb-2">Email</div>
          ...
          <div className="space-y-4 pt-2">
            <div>
              <label htmlFor="firstName" className="block text-sm font-bold text-slate-800 dark:text-slate-200 mb-1">Prénom <span className="text-slate-800 dark:text-slate-200">*</span></label>
            ...
            <div>
              <label htmlFor="lastName" className="block text-sm font-bold text-slate-800 dark:text-slate-200 mb-1">Nom</label>
            ...
            <div>
              <label htmlFor="initials" className="block text-sm font-bold text-slate-800 dark:text-slate-200 mb-1">Initiales</label>
              <input id="initials" type="text" value={initials} onChange={(e) => setInitials(e.target.value)} maxLength={3} placeholder="ex: JB" className="w-full px-3 py-2 border border-slate-300 dark:border-slate-700 rounded-md bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-[#0f4a56]/50 focus:border-[#0f4a56] uppercase transition-all" />
            </div>
            <div>
              <label htmlFor="title" className="block text-sm font-bold text-slate-800 dark:text-slate-200 mb-1">Titre</label>
              <input id="title" type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full px-3 py-2 border border-slate-300 dark:border-slate-700 rounded-md bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-[#0f4a56]/50 focus:border-[#0f4a56] transition-all" />
            </div>
          </div>
        </form>
      );
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative bg-white dark:bg-slate-900 w-full max-w-3xl rounded-lg shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-800">
          ...
          <button type="button" onClick={onClose} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors p-1">
            <X className="w-5 h-5" />
          </button>
        </div>

        <div className="flex px-6 pt-4 border-b border-slate-200 dark:border-slate-800 gap-2">
          <button type="button" onClick={() => setActiveTab('info')} className={`px-4 py-2 text-sm font-semibold rounded-t-md transition-colors ${activeTab === 'info' ? 'bg-[#0f4a56] text-white' : 'text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800'}`}>Informations</button>
          <button type="button" onClick={() => setActiveTab('auth')} className={`px-4 py-2 text-sm font-semibold rounded-t-md transition-colors border border-b-0 border-transparent ${activeTab === 'auth' ? 'bg-[#0f4a56] text-white' : 'border-slate-200 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800'}`}>Authentification forte</button>
        </div>
