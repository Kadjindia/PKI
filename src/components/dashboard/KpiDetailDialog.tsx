...
// --- FONCTIONS UTILITAIRES (Pour éviter les ternaires imbriqués et les fuites d'objets) ---

function formatLatestValue(val: number | undefined, isPercentage: boolean): string {
  if (val === undefined) return "—";
  return isPercentage ? `${val}%` : val.toLocaleString("fr-FR");
}

function formatTrendText(trend: number, trendPercent: number): string {
  if (trend === 0) return "Stable";
  return trend > 0 ? `+${trendPercent}%` : `${trendPercent}%`;
}

function TrendIcon({ trend }: Readonly<{ trend: number }>) {
  if (trend > 0) return <TrendingUp className="w-3.5 h-3.5 text-success" />;
  if (trend < 0) return <TrendingDown className="w-3.5 h-3.5 text-destructive" />;
...
function DetailsTabContent({ details, source }: Readonly<{ details: any[]; source: any }>) {
  if (!details || details.length === 0) {
    return <p className="text-sm text-muted-foreground py-6 text-center">Aucune donnée détaillée disponible pour cet indicateur.</p>;
  }
    ...
    <div className="space-y-2 mt-2">
      ...
      <div className="rounded-lg border border-border overflow-hidden">
        <Table>
          ...
          <TableBody>
            {details.map((row) => (
              // Correction SonarQube : row.label utilisé comme key au lieu de l'index i
              <TableRow key={row.label || safeStringify(row)} className="hover:bg-secondary/20">
...
function SourceTabContent({ source, setUploadOpen }: Readonly<{ source: any; setUploadOpen: (v: boolean) => void }>) {
  if (!source) {
    return (
  ...
  const SourceIcon = SOURCE_ICONS[source.type] || Globe;

  return (
    <div className="space-y-4 mt-2">
      <div className="glass-panel p-4 space-y-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ backgroundColor: `${DATA_SOURCE_COLORS[source.type as DataSourceType]}20` }}>
            <SourceIcon className="w-5 h-5" style={{ color: DATA_SOURCE_COLORS[source.type as DataSourceType] }} />
          </div>
          <div>
            <div className="text-sm font-medium text-foreground">{source.label}</div>
            <div className="text-xs text-muted-foreground">{DATA_SOURCE_LABELS[source.type as DataSourceType]}</div>
          </div>
        </div>
        ...
        {source.fileName && (
          <button type="button" className="flex items-center gap-2 text-xs text-primary hover:underline bg-transparent border-none p-0 cursor-pointer">
        ...
        <div className="glass-panel p-4">
          ...
          <div className="rounded-lg border border-border overflow-auto max-h-[200px]">
            <Table>
              ...
              <TableBody>
                {source.rawData.map((row: any) => {
                  // Génération d'une clé unique basée sur le contenu de la ligne pour éviter l'index
...
function HistoryTabContent({ entries, isPercentage }: Readonly<{ entries: any[]; isPercentage: boolean }>) {
  return (
              ...
              <TableRow key={entry.id} className="hover:bg-secondary/20">
                ...
                <TableCell className="text-xs">
                  <span className="inline-flex items-center gap-1">
                    <EntryIcon className="w-3 h-3" style={{ color: entry.source ? DATA_SOURCE_COLORS[entry.source.type as DataSourceType] : undefined }} />
                    {entry.source ? DATA_SOURCE_LABELS[entry.source.type as DataSourceType] : "—"}
        ...
        <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto bg-card border-border">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <SummaryCard
              ...
              value={formatLatestValue(latestValue, isPercentage)}
            />
            <SummaryCard
              label="Tendance"
              value={formatTrendText(trend, trendPercent)}
              icon={<TrendIcon trend={trend} />}
            />
            <SummaryCard label="Périodes" value={`${entries.length}`} />
            <SummaryCard
              label="Source"
              value={source ? DATA_SOURCE_LABELS[source.type as DataSourceType] : "—"}
              icon={<SourceIcon className="w-3.5 h-3.5" style={{ color: source ? DATA_SOURCE_COLORS[source.type as DataSourceType] : undefined }} />}
            />
          </div>
          ...
          <Tabs defaultValue="evolution" className="mt-2">
            ...
            <TabsContent value="evolution">
              ...
              {/* Correction SonarQube : fuite de booléen */}
              {!!kpi.target && (
            ...
            <TabsContent value="details">
              <DetailsTabContent details={details} source={source} />
            </TabsContent>

            <TabsContent value="source">
              <SourceTabContent source={source} setUploadOpen={setUploadOpen} />
            </TabsContent>

            <TabsContent value="history">
              <HistoryTabContent entries={entries} isPercentage={isPercentage} />
            </TabsContent>
          </Tabs>
        </DialogContent>
      </Dialog>
